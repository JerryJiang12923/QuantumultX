...
const names = $persistentStore.read("chname") ? $persistentStore.read("chname").split(",") : [];
const appids = $persistentStore.read("chappid") ? $persistentStore.read("chappid").split(",") : [];
...
if (typeof $response == "undefined") {
  ...
  for (let i = 0; i < names.length && i < appids.length; i++) {
    ...
    if (forever) {
      data = {
        ...data,
        "purchase_date": "2025-04-09T09:09:09Z"
      };
    } else {
      data = {
        ...data,
    ...
    pyer2.subscriber.subscriptions[appid] = {  
      ...data,
      "original_purchase_date": "2025-04-09T09:09:09Z",
      "store": "app_store",
